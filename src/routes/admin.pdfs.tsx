...
interface Pdf {
  ...
  subject_id: string | null; unit_id: string | null;
  year: number | null; tags: string[];
...
function PdfsPage() {
  ...
  const [filter, setFilter] = useState<{ year?: string; subject?: string }>({});
  ...
  const onDrop = useCallback(async (files: File[]) => {
    ...
    load();
  }, [user, bulkSubject, bulkUnit, bulkTags, subjects]);
      ...
      <Card>
        <CardContent className="pt-4 space-y-4">
          ...
          >
            ...
            <p className="text-xs text-muted-foreground mt-1">Files inherit Subject / Unit / Year from the selectors above.</p>
          </div>
        </CardContent>
      </Card>

      {/* Filters */}
      <Card><CardContent className="pt-4 grid sm:grid-cols-4 gap-2">
        ...
        <Select value={filter.subject ?? "all"} onValueChange={(v) => setFilter({ ...filter, subject: v === "all" ? undefined : v })}>
          <SelectTrigger><SelectValue placeholder="Subject" /></SelectTrigger>
          ...
          {editing && (
            <div className="space-y-3">
              ...
              <div><Label>Subject</Label>
                <Select value={editing.subject_id ?? "none"} onValueChange={(v) => setEditing({ ...editing, subject_id: v === "none" ? null : v, unit_id: null })}>
